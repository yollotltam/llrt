...
use rquickjs::{Error, IntoJs, String as JsString};
use std::io::Write;
...
pub async fn runtime(ctx: &Ctx<'_>) -> Result<(), rquickjs::Error> {
    ...
    if js_init.is_function() {
        ...
        let _ = js_init_tasks.set(idx, js_call)?;
    }
    ...
    if init_tasks_sze == 1 {
        let _ = ctx.eval::<(), _>("await initTasks[0]")?;
    } else if init_tasks_sze > 1 {
        let _ = ctx.eval::<(), _>("await Promise.all(initTasks)")?;
    }
    ...
    Ok(start_process_events(handler, base_url.as_str(), ctx).await?)
}
...
fn get_handler_env() -> Option<String> {
    match get_env(LAMBDA_HANDLER) {
        ...
        None => match get_env(_HANDLER) {
            Some(_handler) => Some(_handler),
            None => None,
        },
    }
}
